...
const problemSchema = z.object({
  ...
  tags: z.enum(['array', 'linkedList', 'graph', 'dp' , "stack" , "function"]),
...
const steps = [
  { id: 1, title: 'Problem Details', icon: FileText, description: 'Basic information and description' },
  { id: 2, title: 'Test Cases', icon: TestTube, description: 'Visible and hidden test cases' },
  { id: 3, title: 'Code Templates', icon: Code, description: 'Initial code and solutions' },
  { id: 4, title: 'Review & Submit', icon: Settings, description: 'Final review and submission' }
];
...
function CreateProblem() {
  ...
  const renderStepContent = () => {
    switch (currentStep) {
      case 1:
        return <ProblemDetailsStep register={register} errors={errors} watchedValues={watchedValues} />;
      case 2:
        return (
          <TestCasesStep
            register={register}
    ...
    <div className="min-h-screen bg-slate-900 text-white p-6">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-slate-400">Design challenging problems for the coding community</p>
        </div>
        ...
        <div className="mb-8">
          ...
          <div className="relative">
                  ...
                  {index < steps.length - 1 && (
                    <div className="flex-1 mx-4">
                      <div className={`
                        ...
                        ${currentStep > step.id ? 'bg-orange-500' : 'bg-slate-600'}
                      `} />
                    </div>
                  )}
                </React.Fragment>
            ...
            <div className="flex items-start gap-10 justify-between mt-4">
              {steps.map((step) => (
                <div key={step.id} className="flex flex-col items-center text-center max-w-[200px]">
                  <h3 className={`font-medium text-sm mb-1 ${currentStep >= step.id ? 'text-white' : 'text-slate-400'}`}>
                    {step.title}
                  </h3>
      ...
      <div className="grid md:grid-cols-2 gap-6">
        ...
        <div>
          ...
          {watchedValues.difficulty && (
            <div className="mt-2">
              <span className={`inline-block px-3 py-1 rounded-full text-xs font-medium text-white ${difficultyColors[watchedValues.difficulty]}`}>
        ...
        <div>
          ...
          {watchedValues.tags && (
            <div className="mt-2">
              <span className={`inline-block px-3 py-1 rounded-full text-xs font-medium text-white ${tagColors[watchedValues.tags]}`}>
                {watchedValues.tags === 'linkedList' ? 'LINKED LIST' : 
                 watchedValues.tags === 'dp' ? 'DYNAMIC PROGRAMMING' : 
...
function TestCasesStep({ register, errors, visibleFields, hiddenFields, appendVisible, appendHidden, removeVisible, removeHidden }) {
  return (
...
function CodeTemplatesStep({ register, errors }) {
  const languages = [
    { name: 'C++', icon: '🔧', color: 'border-blue-500' },
  ...
  return (
    <div className="space-y-8">
      ...
      {languages.map((lang, index) => (
        <div key={lang.name} className={`border-l-4 ${lang.color} bg-slate-700 rounded-lg p-6`}>
          ...
          <div className="space-y-6">
            <div>
              <label className="block text-sm font-medium mb-2">Initial Code Template</label>
      ...
      <div className="grid md:grid-cols-2 gap-8">
          ...
          <div className="bg-slate-700 rounded-lg p-6">
            ...
            <div className="space-y-3">
              <div>
                ...
                <p className="font-medium">{watchedValues.title || 'Untitled Problem'}</p>
              </div>
              <div>
                <span className="text-slate-400 text-sm">Description:</span>
                <p className="text-sm mt-1">{watchedValues.description || 'No description provided'}</p>
              </div>
              <div className="flex gap-2">
                <span className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-medium text-white ${difficultyBadge.color}`}>
                  {difficultyBadge.emoji} {watchedValues.difficulty?.toUpperCase()}
                </span>
                <span className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-medium text-white ${tagBadge.color}`}>
          ...
          <div className="bg-slate-700 rounded-lg p-6">
            ...
            <div className="space-y-2">
              <div className="flex justify-between">
                ...
                <span className="font-medium">{watchedValues.visibleTestCases?.length || 0}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-slate-400">Hidden Test Cases:</span>
                <span className="font-medium">{watchedValues.hiddenTestCases?.length || 0}</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-6">
                ...
                <div key={lang} className="flex items-center justify-between">
                  ...
                  <div className="flex gap-2">
                    <span className={`w-3 h-3 rounded-full ${
                      watchedValues.startCode?.[index]?.initialCode ? 'bg-green-500' : 'bg-red-500'
                    }`}></span>
                    <span className={`w-3 h-3 rounded-full ${
                      watchedValues.referenceSolution?.[index]?.completeCode ? 'bg-green-500' : 'bg-red-500'
                    }`}></span>
                  </div>
                </div>
              ))}
          ...
          <div className="bg-gradient-to-r from-orange-500/10 to-orange-600/10 border border-orange-500/20 rounded-lg p-6">
            <h3 className="text-lg font-medium mb-2 text-orange-400">Ready to Submit?</h3>
            <p className="text-sm text-slate-300">
              Please review all the information above. Once submitted, the problem will be available 
              for users to solve. Make sure all test cases are correct and code templates are properly formatted.
            </p>
          </div>
        </div>
      </div>
